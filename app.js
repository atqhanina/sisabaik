...
async function siapkanKatalog() {
    ...
    try {
        const respons = await fetch("penawaran.json");
...
perbaruiTampilan();
    ...
    function renderKartu(data, wadah, status, keranjang) {
        ...
        penawaran.setCustomValidity(hargaPenawaran > hargaNormal ? "Harga penawaran tidak boleh melebihi harga normal." : "");
         }

         normal.addEventListener("input", validasiHarga);
         penawaran.addEventListener("input", validasiHarga);
         form.addEventListener("submit", (event) => {validasiHarga();
